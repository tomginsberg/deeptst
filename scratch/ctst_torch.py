...
if __name__ == '__main__':
    ...
    for seed in range(100):
        ...
        n = int(
            N * tr.test(model, DataLoader(d2, batch_size=N, shuffle=False, num_workers=4), verbose=False)[0]['val_acc'])
        test = binom_test(n, N, 0.5)
        tests.append(test)

    res = np.array(tests) <= args.alpha
    res = np.mean(res), np.std(res) / np.sqrt(100)
    print(res)
    np.save(f'results/{args.run_name}/res_{N=}.npy', np.array(res))
    np.save(f'results/{args.run_name}/tests_{N=}.npy', np.array(tests))
