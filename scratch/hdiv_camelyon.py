...
from data.sample_data.camelyon import camelyon
...
def evaluate(samples=max_data_size, seed=0):
    path = join(args.data_dir, f'{samples=}_{seed=}.npy')
    scores = []
    camelyon_train = split_dataset(camelyon(split='id_val'), num_samples=samples, random_seed=seed)
    camelyon_test = split_dataset(camelyon(split=args.split), num_samples=samples, random_seed=seed)

    for rep in range(100):
        try:
            indices = torch.randperm(len(camelyon_train))[:samples]
            data1 = torch.stack([camelyon_train[i][0] for i in indices], dim=0)

            data2 = torch.stack([camelyon_test[i][0] for i in range(samples)], dim=0)
